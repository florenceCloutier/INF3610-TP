...
NET_DEV_CFG_ETHER  NetDev_GEM_1 = {
    ...
    1536u,                          /* Desired size      of device's large receive  buffers (in octets) [see Note #2].  */
      64u,                          /* Desired number    of device's large receive  buffers             [see Note #3a]. */
    ...
    1632u,                          /* Desired size      of device's large transmit buffers (in octets) [see Note #2].  */
      64u,                          /* Desired number    of device's large transmit buffers             [see Note #3b]. */
     160u,                          /* Desired size      of device's small transmit buffers (in octets) [see Note #2].  */
      32u,                          /* Desired number    of device's small transmit buffers             [see Note #3b]. */
    ...
    NET_DEV_CFG_FLAG_NONE,          /* Desired option flags, if any (see Note #6).                                      */
            ...
            32u,                    /* Desired number of device's receive  descriptors (see Note #7).                   */
            16u,                    /* Desired number of device's transmit descriptors.                                 */


    0xE000B000u,                    /* Base address   of device's hardware/registers.                                   */

             0u,                    /* Size           of device's data bus (in bits), if available.                     */


   "50:E5:49:E6:8D:28",             /* Desired device hardware address; may be NULL address or string ...               */
                                    /* ... if  device hardware address configured or set at run-time.                   */
};

const  NET_PHY_CFG_ETHER  NetPhy_Cfg_Ether_1 = {
    0xFF,                             /* Phy bus address.                                                                 */
    NET_PHY_BUS_MODE_GMII,          /* Phy bus mode.                                                                    */
    NET_PHY_TYPE_INT,               /* Phy type.                                                                        */
    NET_PHY_SPD_1000,               /* Auto-Negotiation determines link speed.                                          */
