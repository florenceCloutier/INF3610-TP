...
const  NET_DEV_BSP_ETHER     NetDev_BSP_AXIEthernetLite_0 = {                               /* Board-/device-specific BSP fnct ptrs : */
                                 ...
                                 NET_ERR  *p_err)
{
    ...
    int_id = UCOS_AXIETHERNETLITE_ConfigTable[UCOS_ETHERNET_DEVICE_ID].IntSource;
                                       ...
                                       NET_ERR  *p_err)
{
   ...
   (void)&p_if;                                                  /* Prevent 'variable unused' compiler warning.          */
    ...
    clk_freq = 0;/* $$$$ Insert code to return each network interface's/device's clock frequency. */
   *p_err     = NET_DEV_ERR_NONE;
