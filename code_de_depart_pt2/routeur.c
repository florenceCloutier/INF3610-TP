...
void TaskReset(void *data) {
	...
	int i;
	while (true) {
		// RDV unilatéral via la sémaphore SemReset
		// À completer
		// xil_printf("------------------- Task Reset --------------------\n");
		routerIsOn = 1; // Le système (re)démarre

		// Allumez les LEDS (0x03, 0x04, etc.) pour montrer qu’on est en mode
		// fonctionnement
		// À compléter
		// Rédémarrerztoutes les tâches (suspendues précédemment dans TaskStartUp ou TaskStop)
		// À compléter
	}
}
...
void TaskStop(void *data) {
	...
	OS_ERR err;
	while (true) {
		// RDV unilateral avec la sémaphore SemStop
		// À compléter
		if (!routerIsOn) {
			continue ;
		}
		...
		// À compléter
		routerIsOn = 0;
		// Afficher les statistiques et éteindre les LEDS (0x03, 0x04, etc.)
		// pour montrer qu’on s’arrête
		// À compléter
...
void gpio_isr(void *p_int_arg, CPU_INT32U source_cpu){
	...
	// On regarde quel bouton a été activé et on l’affiche
	// Selon le, cas on redémarre les tâches du système ou on redémarre
	// StartupTaskTCB qui doit s’occuper d’arrêter le système
	// À compléter
